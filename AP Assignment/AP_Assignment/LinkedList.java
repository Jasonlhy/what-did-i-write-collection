...
/**
 * Title: LinkedList.java Description: Contains classes for linked list Company:
 * ICT HKIVE(TY)
 * 
 * @author modified by Liu Ho Yin
 * 
 *         (for Lab03)
 */
...
public class LinkedList implements Serializable{
	...
	public void add(int n, Object item) {
		...
		ListNode current = head;
		for (int i = 1; i < n - 1; i++)
			// current will point to n-1th node
			current = current.getNext();
	...
	public static String displayCombo(LinkedList list) {
		String s = "\r";
		for (int i = 1; i <=list.count(); i++){
			if (i==1)
				s += "+"+(Integer) list.get(i)+"\n";
			else 
				s+=i+" Combo!! +"+(Integer) list.get(i)+"\n";
		}
		return s;
	}
