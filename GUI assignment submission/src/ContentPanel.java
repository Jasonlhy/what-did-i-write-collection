...
public class ContentPanel extends JPanel implements Observer{
    private JPanel cardPanel;  // holding tabbedPane and desktopPane ith between two view
	...
	if (status==FileRecord.ADD_NEW_FILE){
	    //frecth the file reference from the model and add contents into view
	...
	else if (status==FileRecord.REMOVE_FILE){ // the model remove a file , and the index not supposed to be here is going to be removed
	...
	else if (status==FileRecord.CHANGE_FILE){ // the model remove a file , change the index title
	    // get the new title
	...
	else if(status ==FileRecord.REMOVE_ALL) { // fileRecord remove all file
    ...
    /* return a seleced index based on the view using , this index is basd on the order of file creation 
     * therefore the index is the same for the "same" doucment in different view */
	...
	// no selection
	if (selectedScrollPane==null) return null;
    ...
    /* get all the component for editing 
     * we only return the component in the jtabbedpane
     * because the content in the jtabbedpane and jinternal frame is the sasme
     * but it is much more easy to get the component from jtabbedpane*/
