...
public class IOSystem{
	...
	String line;
	try {
	    maxColumn =0; // variable that tell the array ho wmuch it is required
	    reader = new BufferedReader(new FileReader(file));
	    while((line=reader.readLine())!=null){
		...
		for (int i=0; i<cells.length; i++) {
			...
			if (j>=cellsInLine.length) // the string [] is going index out of bound
			    cells[i][j]="";
			else 
	...
	if (c instanceof JTextPane){
	    ...
	    //since the table size may be bigger than the value contain in the file
	    //we need to check out what value needed to be outputed
	    //the counter is about the exact max row and column index we need to output
	    int maxColumnIndex=0;
	    int maxRowIndex=0;

	    // still have a word in a line && column index is bigger than the counter-> expand the coulumn
	    // still have a word in a line && row index is bigger than the counter -> expand the row couter
