...
public class InformationPanel extends JTabbedPane {
	...
	public void mouseClicked(MouseEvent e) {
		...
		// get the component that tree is holding -> get the object in mutable node-> get the file in file noe
...
class FileNode {
    ...
    public FileNode(File f) { this.f = f; }

    public File getFile() { return f; }

    public String toString() {
	return f.getName().length() > 0 ?
	    f.getName() : f.getPath();
    }
	...
	if (!(obj instanceof Boolean)) return; // Already expanded
	...
	for (int k=0; k<files.length; k++) {
	    File f = files[k];
	    if (!(FileNode.isSelectableFile(f) || f.isDirectory())) continue;

	    FileNode nd = new FileNode(f);
	    DefaultMutableTreeNode node =
		new DefaultMutableTreeNode(nd);
	    parent.add(node);
...
class FileTreeCellRenderer extends DefaultTreeCellRenderer {
    ...
    public Component getTreeCellRendererComponent(JTree fileTree,
	    Object value, boolean sel, boolean expanded, boolean leaf,
	    int row, boolean hasFocus) {

	super.getTreeCellRendererComponent(
		fileTree,value,sel,expanded,leaf,row,hasFocus);
