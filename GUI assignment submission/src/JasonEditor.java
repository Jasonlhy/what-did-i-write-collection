...
public class JasonEditor extends JFrame {
	...
	// column action
	Action addLeftColumnAction= new AddLeftColumnAction("add left column",new ImageIcon("../graph/left_column.PNG"));
	Action addRightColumnAction= new AddRightColumnAction("add right column",new ImageIcon("../graph/right_column.PNG"));

	//remove action
	Action removeRowAction= new RemoveRowAction("remove row",new ImageIcon("../graph/remove_row.PNG"));
	Action removeColumnAction= new RemoveColumnAction("remove column",new ImageIcon("../graph/remove_column.PNG"));
	...
	size.addActionListener(new ActionListener(){
		...
		// actively involve actionperformed by passing the event to sizeAction
	...
	if (value == JFileChooser.APPROVE_OPTION){
	    ...
	    // a cache is need because java.exists() for a new file is not worakble
	...
	for (int i=0;i<component.length;i++){
		...
		if (enable)
		    target.setText((String) (target.getAction())
    ...
    /* get the source (JList) , change in selection -> get the index ,based on it to select a file*/
	...
	@Override
		...
		if (index>=0){// have content
		    // for safety
		    int value = JOptionPane.showConfirmDialog(null,"Want to save the file before leave?","Want to save the file first?",JOptionPane.YES_OPTION);
	...
	public void actionPerformed(ActionEvent e){
		...
		int firstSelectedColumn = table.getSelectedColumn(); // a flag to determine if there is any selection 
	...
	public void actionPerformed(ActionEvent e){
		...
		int firstSelectedColumn = table.getSelectedColumn(); // a flag to determine if there is any selection 
		if (firstSelectedColumn == -1){ // no selection , add to the righest side
		    // find the last table colum index , and the new indx to be added to the right side will be increase by 1
		    int newIndex = table.getColumnCount()-1 +1 ;
		    model.addColumn(newIndex,1);
		}
		else{
		    int []columns = table.getSelectedColumns();
		    // find the last selected table index , and the new index to be added to its right side will be increase by 1
	...
	public void actionPerformed(ActionEvent e){
		...
		int firstSelectedRow = table.getSelectedRow(); // a flag to determine if there is any selection
		if (firstSelectedRow == -1)  // no selection-> default add to the toppest
		    model.addRow(0,1);
		else {
		    int [] rows = table.getSelectedRows();
		    // find the first selected table row index, and the new index is the same as it
	...
	public void actionPerformed(ActionEvent e){
		...
		if (firstSelectedRow == -1)  {
		    // get the index of last row in table, the new added row index increase by 1
		    int firstNewIndex = table.getRowCount()-1+1;
		    model.addRow(firstNewIndex,1);
		}// no selection-> default add to the toppest
		else {
		    int [] rows = table.getSelectedRows();
		    // find the last selected table row index, and the new added row index increase by 1
	...
	public void actionPerformed(ActionEvent e){
		...
		if (model.getColumnCount()==table.getSelectedColumns().length)
		    ...
		    int firstSelectedColumn= table.getSelectedColumn(); // a flag to determine if there is any selection
		    if (firstSelectedColumn== -1)  // no selection-> default add to the toppest
	...
	public void actionPerformed(ActionEvent e){
	    int value = JOptionPane.showConfirmDialog(null,"Want to save the file ?","Want to save all the file?",JOptionPane.YES_OPTION);
	    if (value==JOptionPane.YES_OPTION){
		Action saveAllAction= new SaveAllAction("Save All", new ImageIcon("../graph/saveAs.gif"), KeyEvent.VK_A);
